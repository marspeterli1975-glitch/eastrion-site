...
import { buildDemoReport, saveRiskReportToStorage } from "@/lib/risk-report-storage";
...
export default function RiskAtlasPage() {
  ...
  saveRiskReportToStorage(demoReport);
  router.push("/riskatlas/report");
  }
...
async function handleScan() {
  ...
  try {
    ...
    const enrichedReport = {
      ...
      route_name: country.trim()
        ? `Origin → ${country.trim()}`
        : demoReport.route_name,
      product_name: industry.trim() || demoReport.product_name,
      generated_at: new Date().toISOString(),
      report_id: `RA-PREVIEW-${Date.now()}`,
      executive_summary: `This preview was generated for ${companyName.trim()}. The route into ${country.trim()} under ${industry.trim()} conditions shows manageable exposure, but requires structured execution.`,
      decision_note: `Proceed with controlled execution. Validate supplier readiness, inland constraints, and loading plan before booking.`,
    };

    saveRiskReportToStorage(enrichedReport);

    router.push("/riskatlas/report");

  } catch (err) {
    console.error(err);
    setError("Local generation failed.");
  } finally {
    setLoading(false);
  }
}
